...
Input types
  ...
  CSV file
    ...
    or --album-col. Rows that do not have any text in the title column will be treated as album
    downloads.
  ...
  Spotify
    ...
    Using Credentials
      ...
      Start sldl with the obtained credentials and an authorized action to trigger the Spotify app
      login flow:

      sldl spotify-likes --spotify-id 123456 --spotify-secret 123456 -n 1 --print-tracks

      sldl will try to open a browser automatically but will fallback to logging the login flow URL
      to output. After login flow is complete sldl will output a token and refresh token and finish
  ...
  Search string
    ...
    String input accepts a shorthand for track and album downloads: The input ARTIST - TITLE will be
    parsed as artist=ARTIST, title=TITLE when downloading songs, and artist=ARTIST, album=TITLE when
    run with --album.
...
Download modes
  ...
  Aggregate
    With -g/--aggregate, sldl performs an ordinary search for the input, then attempts to group the
    results into distinct songs and download one of each, starting with the one shared by the most
    users. Note that --min-shares-aggregate is 2 by default, meaning that songs shared by only one
    user will be ignored. Aggregate mode can be used to download all songs by an artist.

  Album Aggregate
    Activated when both --album and --aggregate are enabled. sldl will group shares and download one
...
File conditions
  ...
  Note on availability of metadata
    ...
    set, then sldl will still accept any file with unknown bitrate. To reject all files where one or
    more of the checked properties is null (unknown), enable --strict-conditions.
    ...
    bitrate, enabling --strict-conditions and setting a --min-bitrate will make sldl ignore all
...
Configuration
  ...
  Syntax
    ...
    user directory (even on windows). The path variable {bindir} stores the directory of the sldl
    binary.
